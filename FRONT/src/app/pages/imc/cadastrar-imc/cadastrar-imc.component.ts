  ...
  export class CadastrarImcComponent{
    ...
    ngOnInit() : void {
      this.client
        .get<Usuario[]>("https://localhost:4200/api/imc/listar")
          .subscribe ({
            next: (usuarios) => {
    ...
    cadastrar() : void {
      ...
      this.client
        .post<Imc>("https://localhost:4200/api/imc/cadastrar", imc)
          .subscribe ({
            next: (imc) => {
              this.snackbar.open("O Imc foi cadastrado com sucesso!", "Felipe_Lima_DB", {
                duration : 1500,
